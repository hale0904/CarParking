...
const migrateLegacyLane = (lane) => {
  ...
  return {
    id: lane.id,
    points: [lane.x, lane.y, lane.x + len * Math.cos(rad), lane.y + len * Math.sin(rad)],
...
const ParkingMapEditor = () => {
  ...
  const setZones = (val) =>
    updateActiveFloor((f) => ({ zones: typeof val === 'function' ? val(f.zones) : val }));
  const setStandaloneSlots = (val) =>
    updateActiveFloor((f) => ({
      standaloneSlots: typeof val === 'function' ? val(f.standaloneSlots) : val,
    }));
  const setLaneNodes = (val) =>
    updateActiveFloor((f) => ({ laneNodes: typeof val === 'function' ? val(f.laneNodes) : val }));
  const setLaneEdges = (val) =>
    updateActiveFloor((f) => ({ laneEdges: typeof val === 'function' ? val(f.laneEdges) : val }));
  const setEntrances = (val) =>
    updateActiveFloor((f) => ({ entrances: typeof val === 'function' ? val(f.entrances) : val }));
  const setExits = (val) =>
    updateActiveFloor((f) => ({ exits: typeof val === 'function' ? val(f.exits) : val }));
  ...
  const updateSlotGroupDimensions = (group, newWidth, newHeight, zone = null) => {
    ...
    let count = 0;
    if (isHorizontal) {
      count = Math.max(1, Math.floor((newWidth + SLOT_GAP) / (SLOT_SIZE.width + SLOT_GAP)));
    } else {
      count = Math.max(1, Math.floor((newHeight + SLOT_GAP) / (SLOT_SIZE.height + SLOT_GAP)));
    }
    ...
    return {
      ...
      width: isHorizontal ? count * SLOT_SIZE.width + (count - 1) * SLOT_GAP : SLOT_SIZE.width,
      height: isHorizontal ? SLOT_SIZE.height : count * SLOT_SIZE.height + (count - 1) * SLOT_GAP,
      slots: newSlots,
    };
  };
  ...
  const handleUpdateNode = (id, changes) =>
    setLaneNodes((prev) => prev.map((n) => (n.id === id ? { ...n, ...changes } : n)));
  ...
  const handleUpdateEdge = (id, changes) =>
    setLaneEdges((prev) => prev.map((e) => (e.id === id ? { ...e, ...changes } : e)));
    ...
    const handleKeyDown = (e) => {
      if (editorMode === 'DRAW_BOUNDARY') {
        ...
        if (e.key === 'Escape') {
          ...
          const lastEdge = [...laneEdges]
            .reverse()
            .find((edge) => edge.toNodeId === currentNodeId || edge.fromNodeId === currentNodeId);

          if (lastEdge) {
            // The previous node is the other end of that edge
            const prevNodeId =
              lastEdge.fromNodeId === currentNodeId ? lastEdge.toNodeId : lastEdge.fromNodeId;
  ...
  if (selectedEntity) {
    if (selectedEntity.type === 'ZONE') {
      ...
      if (selectedEntity.parentId) {
        ...
        if (selectedData)
          selectedData = { ...selectedData, parentGroupId: selectedEntity.parentId }; // Inject parent ID for display
      } else {
      ...
      if (selectedData) {
        selectedData._connectedCount = laneEdges.filter(
          (e) => e.fromNodeId === selectedData.id || e.toNodeId === selectedData.id,
        ).length;
      }
  ...
  const handlePropertiesUpdate = (id, props) => {
    ...
    if (selectedEntity.type === 'ZONE') handleUpdateZone(id, props);
      ...
      handleUpdateSlotGroup(selectedEntity.parentId, id, props);
    else if (selectedEntity.type === 'SLOT' && !selectedEntity.parentId)
      handleUpdateStandaloneSlot(id, props);
    else if (selectedEntity.type === 'SLOT' && selectedEntity.parentId)
      handleUpdateGroupedSlot(selectedEntity.grandParentId, selectedEntity.parentId, id, props);
  ...
  const assignedSensorIds = Array.from(
    new Set(
      floors
        .flatMap((f) => [
