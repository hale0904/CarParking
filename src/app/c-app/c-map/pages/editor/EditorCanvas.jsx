...
const EditorCanvas = ({
  ...
  useEffect(() => {
    ...
    if (selectedEntity.type === 'SLOT' && selectedEntity.parentId) {
      ...
      selectedEntity.type === 'LANE' ||
      selectedEntity.type === 'LANE_EDGE' ||
      selectedEntity.type === 'LANE_NODE'
    ) {
      transformerRef.current.nodes([]);
    } else {
        ...
        <Stage
          ...
          onMouseDown={(e) => {
            ...
            if (editorMode === 'DRAW_BOUNDARY') {
              ...
              const SNAP_DIST = 18;
              const nearNode = laneNodes?.find(
                (n) => Math.hypot(n.x - x, n.y - y) < SNAP_DIST / scale,
              );
            ...
            {zones.map((zone) => {
              ...
              return (
                <Group key={zone.id} id={zone.id}>
                    ...
                    (() => {
                      ...
                      return (
                        <Group listening={false}>
                          ...
                          />
                          <Text
                            text={`${realH}${parkingUnit}`}
                            x={bbox.x + bbox.w + 5}
                            y={bbox.y + bbox.h / 2 - fs / 2}
                            fontSize={fs}
                            fill="#374151"
                          />
                        </Group>
                  ...
                  {zone.slotGroups &&
                    zone.slotGroups.map((group) => renderSlotGroup(group, zone.id))}
                </Group>
              );
            })}
