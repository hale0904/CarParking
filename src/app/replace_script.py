...
file_path = r"d:\Dev\DoAn\CarParking\src\app\c-app\c-map\pages\ParkingMapEditor.jsx"

with open(file_path, "r", encoding="utf-8") as f:
    content = f.read()

# Chunk 1: parseLegacyLanesToGraph
chunk1_target = """const migrateLegacyLane = (lane) => {
...
chunk3_target = """                            lanes: (floor.lanes || []).map(l => migrateLegacyLane({
                                id: l.code,
...
chunk3_replacement = """                            ...parseLegacyLanesToGraph(floor.lanes || []),"""
...
chunk7_target = """                } else if ((e.ctrlKey || e.metaKey) && e.key === 'z') {
                    e.preventDefault();
                    handleUndoZonePoint();
                }
            }
...
chunk7_replacement = """                } else if ((e.ctrlKey || e.metaKey) && e.key === 'z') {
                    e.preventDefault();
                    handleUndoZonePoint();
                }
...
# Chunk 8: CRUD
chunk8_target = "const handleUpdateLane = (id, newProps) => {\n        setLanes(prev => prev.map(l => l.id === id ? { ...l, ...newProps } : l));\n    };"

chunk8_replacement = """    const handleAddNode = (node) => setLaneNodes(prev => [...prev, node]);
...
content = content.replace(chunk8_target, chunk8_replacement)

# Chunk 9: handleDrop
chunk9_target = """        } else if (type === 'LANE') {
            const newLane = {
                id: `lane-${Date.now()}`,
...
chunk11_target = "else if (selectedEntity.type === 'LANE') handleUpdateLane(id, props);"
chunk11_replacement = "else if (selectedEntity.type === 'LANE_NODE') handleUpdateNode(id, props);\n        else if (selectedEntity.type === 'LANE_EDGE') handleUpdateEdge(id, props);"
content = content.replace(chunk11_target, chunk11_replacement)

# Chunk 12: handleSave
chunk12_target = """                    lanes: floor.lanes.map((l, i) => {
                        const pts   = l.points || [];
                        const x1    = pts[0] ?? 0;
                        const y1    = pts[1] ?? 0;
...
chunk12_replacement = """                    lanes: floor.laneEdges.map((edge, i) => {
...
chunk13_target = """        if (type === 'LANE') {
            setLanes(prev => prev.filter(l => l.id !== id));
        } else if (type === 'ENTRANCE') {"""

chunk13_replacement = """        if (type === 'LANE_NODE') {
            handleDeleteNode(id);
